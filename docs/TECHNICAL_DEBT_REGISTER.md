...
| **TD-14** | **Descubierto en E3-B:** `pnpm typecheck` reportaba **117 errores**. Progreso de remediación: E4-B (−19: node-fetch + build-scope demo) → **98**; **E4-C1 RC-1 RESUELTO** (−3) → **95**; **E4-C2 RC-2 (Longevity) RESUELTO** (−20) → **75**; **E4-D1 RC-2 (fuera de Longevity) RESUELTO** (−21) → **54**; **E4-D2 RC-4 (barrel) RESUELTO** (−9) → **45**; **E4-D3 RC-8 parcial** (−1) → **44**; **E4-D4 RC-2 residual (api/pipelines) RESUELTO** (−5: 5 `queryOne<RowType>` en pipeline-v2) → **39**. **RC-1/RC-2/RC-4 eliminados por completo.** Pendientes: RC-5 (legacy 9), RC-3 (EventBus 8), RC-8 bloqueados (7, TD-19), TD-18 (funnel `.rows`, 5), RC-7 (Logger 5), RC-6 (paths 3). El runtime usa `transpile-only`, por eso no bloquean ejecución. | Type Check/Build no bloqueantes hasta `typecheck=0`; corregir el resto exige tocar producción (autorización por sprint). | ALTO | **P0** | E4-C2+ (RC-2 keystone) |
