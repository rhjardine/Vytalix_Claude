...
| **TD-14** | **Descubierto en E3-B:** `pnpm typecheck` reportaba **117 errores**. Progreso de remediación: E4-B (−19: node-fetch + build-scope demo) → **98**; **E4-C1 RC-1 RESUELTO** (−3) → **95**; **E4-C2 RC-2 (Longevity) RESUELTO** (−20) → **75**; **E4-D1 RC-2 (fuera de Longevity) RESUELTO** (−21) → **54**; **E4-D2 RC-4 (barrel) RESUELTO** (−9) → **45**; **E4-D3 RC-8 parcial** (−1) → **44**; **E4-D4 RC-2 residual (api/pipelines) RESUELTO** (−5) → **39**; **E4-D5 RC-7 (Logger) RESUELTO** (−3) → **36**; **E4-D6 RC-6 CERTIFIED ZERO** (→ TD-20) → **36**; **E4-D7 RC-3 CERTIFIED ZERO** (0 fixes: 9 emit/on requieren rediseño de EventBus + cambian runtime → TD-21; 2 son scope Logger) → **36**. **RC-1/RC-2/RC-4/RC-7 eliminados.** Pendientes: RC-3→TD-21 (9), Logger (2), RC-5 (legacy 9), RC-8→TD-19 (7), TD-18 (5), RC-6→TD-20 (3), residual legacy (1). El runtime usa `transpile-only`, por eso no bloquean ejecución. | Type Check/Build no bloqueantes hasta `typecheck=0`; corregir el resto exige tocar producción (autorización por sprint). | ALTO | **P0** | E4-C2+ (RC-2 keystone) |
| **TD-21** | **Descubierto/reclasificado en E4-D7:** desconexión arquitectónica de eventos. 9 call-sites usan `eventBus.emit(name,…)`/`eventBus.on(name,…)` con nombres string ad-hoc (`vitality.assessed`, `referral.triggered`, `funnel.*`, `referral.converted`) — pero `eventBus` es `LocalEventBus implements IEventBus` (solo `publish/subscribe`, `emitter` privado). Roto en compilación Y runtime (`emit`/`on` = undefined → TypeError). Emisores (funnel/biological-age/referral/quota) y listeners (`registerPlatformEventListeners` en pipeline-v2, **comentado en server.ts**) forman un sistema ad-hoc paralelo al bus tipado. Reconciliar requiere ADR de arquitectura EventBus + cambio de runtime. NO es rename type-only. **DECISIÓN (A1): [ADR_EVENTBUS.md](./ADR_EVENTBUS.md) — Opción A (bus tipado publish/subscribe como único modelo; NO exponer emit/on). PROPOSED; pendiente de aceptación + input de negocio por-cadena + autorización de runtime. Plan: [EVENTBUS_MIGRATION_PLAN.md](./EVENTBUS_MIGRATION_PLAN.md).** Nota: 2 errores adicionales (`clinicalLog.funnelLead`/`assessmentCompleted`) son del helper Logger (métodos faltantes), scope separado. | Sistema de eventos ad-hoc no funcional; listeners nunca registrados; emisores lanzarían si se ejecutaran. | MEDIO | **P2** | ADR EventBus + sprint funcional (con tests de runtime) |
| **TD-20** | **Descubierto/reclasificado en E4-D6:** 3 imports dinámicos rotos (RC-6) — `billing-admin.handler.ts(99)` `await import('../lib/redis')`, `(118)` `await import('./metering.service')` (además `DEFAULT_UNIT_PRICES_CENTS` NO exportado), `health.handler.ts(71)` `await import('../events/event-bus')`. Fallan en runtime hoy (module-not-found). Corregir el path **cambia el comportamiento** (emite logs antes omitidos, invoca `getRedisClient()`, invierte el resultado del health-check de event_bus, 500→200). NO es corrección type-only. | Imports que fallan en runtime; health-check event_bus reporta no-saludable; log de revocación de API key nunca se emite. | MEDIO | **P2** | Sprint de corrección funcional (con autorización de cambio de runtime + tests) |
| **TD-19** | **Descubierto en E4-D3:** 7 errores RC-8 no corregibles bajo la política estricta (sin unknown-cast, sin cambio de comportamiento, sin cambio de schema/contrato): (a) `.detail`/`.type` sobre `unknown` de `res.json()` en `index.ts`(×3) y `disglobal-client.ts`(×2) — byte-idéntico exige unknown-cast; (b) DTO Zod-opcional→requerido en `external-v2.handler.ts` (EngagementEvent) y `funnel.service.ts` (DimensionalMeasurement ×2) — exige cambiar schema/consumidor. | Type-safety reducida en parseo de errores y borde DTO; sin impacto de runtime. | MEDIO | **P2** | Sprint con decisión de política (narrowing tipado de error body vs ProblemDetailV1; alinear Zod↔dominio) |
