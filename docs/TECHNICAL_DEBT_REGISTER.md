...
| **TD-14** | **Descubierto en E3-B:** `pnpm typecheck` reportaba **117 errores**. Progreso de remediación: E4-B (−19: node-fetch + build-scope demo) → **98**; **E4-C1 RC-1 RESUELTO** (−3) → **95**; **E4-C2 RC-2 (Longevity) RESUELTO** (−20: `rawQuery<T>()` genérico + call-sites Longevity tipados) → **75**. Pendientes: RC-2 fuera de Longevity (~30, ahora migrable sin tocar `db.ts`), RC-3, RC-4, RC-5, RC-7, RC-8. El runtime usa `transpile-only`, por eso no bloquean ejecución. | Type Check/Build no bloqueantes hasta `typecheck=0`; corregir el resto exige tocar producción (autorización por sprint). | ALTO | **P0** | E4-C2+ (RC-2 keystone) |
