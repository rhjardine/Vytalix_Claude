...
| **TD-14** | **Descubierto en E3-B:** `pnpm typecheck` reporta **117 errores en código de producción** (21 archivos: `src/api/handlers/*`, `src/core/referral.engine.ts`, `src/longevity/*`, `src/platform/*`, `src/shared/*`, `src/demo/*`, `src/legacy/*`, `src/index.ts`). El runtime usa `ts-node-dev --transpile-only` (sin typecheck) y vitest usa esbuild, por eso no bloquean ejecución. Type Check/Build permanecen advisory por esta causa, **no** por TD-01. | Type Check y Build no pueden ser bloqueantes; corregir exige tocar producción (prohibido en sprints de gobernanza). | ALTO | **P0** | E3-C (sprint de implementación dedicado) |
