...
| **TD-21** | **Descubierto/reclasificado en E4-D7:** desconexión arquitectónica de eventos. 9 call-sites usan `eventBus.emit(name,…)`/`eventBus.on(name,…)` con nombres string ad-hoc (`vitality.assessed`, `referral.triggered`, `funnel.*`, `referral.converted`) — pero `eventBus` es `LocalEventBus implements IEventBus` (solo `publish/subscribe`, `emitter` privado). Roto en compilación Y runtime (`emit`/`on` = undefined → TypeError). Emisores (funnel/biological-age/referral/quota) y listeners (`registerPlatformEventListeners` en pipeline-v2, **comentado en server.ts**) forman un sistema ad-hoc paralelo al bus tipado. Reconciliar requiere ADR de arquitectura EventBus (tipar los eventos o exponer emit/on) + cambio de runtime. NO es rename type-only. Nota: 2 errores adicionales (`clinicalLog.funnelLead`/`assessmentCompleted`) son del helper Logger (métodos faltantes), scope separado. | Sistema de eventos ad-hoc no funcional; listeners nunca registrados; emisores lanzarían si se ejecutaran. | MEDIO | **P2** | ADR EventBus + sprint funcional (con tests de runtime) |
