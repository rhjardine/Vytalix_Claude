...
|---|---|---|---|---|---|
| **TD-01** | `src/vertical2/app.ts` importa módulos inexistentes (`./admin/adminRouter`, `./shared/middleware/partnerMiddleware`, `./shared/db/db`, `./shared/config/config`). El directorio solo contiene `app.ts` + `db.ts`. | `pnpm typecheck` y `pnpm api:build` fallan; stages 2/3 de CI quedan advisory. | ALTO | **P0** | E2 |
