...
export const CreateExchangeRateSchema = z.object({
  ...
  // Partial record: only include currencies you have rates for.
  // z.record(CurrencySchema, ...) in Zod 4 requires ALL enum values — use string key instead.
  rates: z.record(z.string().max(3).toUpperCase(), z.number().positive()).refine(
    (r) => {
      const validCurrencies = new Set(['USD', 'MXN', 'COP', 'PEN', 'EUR']);
      return Object.keys(r).length >= 1 && Object.keys(r).every(k => validCurrencies.has(k));
    },
    'rates must contain at least one valid currency code (USD, MXN, COP, PEN, EUR)'
  ),
