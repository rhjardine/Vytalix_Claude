...
import { IngestionService } from '../shared/ingestion.service'
import { PipelineOrchestrator } from '../api/pipelines/orchestrator'
...
export async function externalIngestObservations(req: Request, res: Response) {
  ...
  const { withTenant: wt } = await import('../lib/db')
  const patient = await wt(keyContext.tenantId, (tc: any) =>
    tc.queryOne(
