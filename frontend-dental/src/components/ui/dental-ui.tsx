...
/**
 * components/ui/dental-ui.tsx
 *
 * Sistema de componentes UI para CFE Dental.
 * Migrado desde frontend-staging/ui/ui-components.tsx.
 *
 * Cambios respecto al staging:
 *   - MoneyDisplay: locale ya no está hardcodeado a 'es-MX'.
 *     Acepta prop `locale` opcional; usa contexto del Tenant como fallback.
 */
...
// ── Money display ─────────────────────────────────────────────────────────────
//
// FASE 3 FIX: 'es-MX' era hardcodeado. Ahora acepta prop `locale` opcional.
// El locale debe provenir del TenantContext (session.locale).
// Valor por defecto: 'es-MX' para mantener compatibilidad con el staging.

export function MoneyDisplay({
  amount,
  currency = 'MXN',
  locale = 'es-MX',   // ← recibe el locale del Tenant; no hardcodeado
  size = 'md',
}: {
  amount: number;
  currency?: string;
  locale?: string;    // ← nuevo prop para internacionalización delegada al Tenant
  size?: 'sm' | 'md' | 'lg';
}) {
