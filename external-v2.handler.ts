...
import { z } from 'zod'
import { BiologicalAgeService, BiophysicsAssessRequestSchema } from '../biological-age/biological-age.service'
...
async function resolveApiKey(apiKey: string): Promise<ApiKeyContext | null> {
  ...
  // Direct DB query — bypass tenant RLS for key lookup (keys table has no tenant filter)
  try {
    const { getDb } = await import('../lib/db')
    const db = getDb()
    const row = await db.rawQueryOne(
      `SELECT id, "tenantId", permissions, "rateLimitTier"
       FROM api_keys
       WHERE "keyHash"=$1 AND "isActive"=true
    ...
    db.rawQuery(
      `UPDATE api_keys SET "lastUsedAt"=NOW(), "requestCount"="requestCount"+1 WHERE id=$1`,
      [row.id]
    ).catch(() => {})
...
function apiKeyAuth(scope: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    const key = req.headers['x-api-key'] as string | undefined
    if (!key) {
      return res.status(401).json(problemDetail(401, 'Missing X-API-Key header', req.correlationId))
    }

    const ctx = await resolveApiKey(key)
    if (!ctx) {
      return res.status(401).json(problemDetail(401, 'Invalid or revoked API key', req.correlationId))
    }

    // Check scope permission
    const [resource, action] = scope.split(':')
    const allowed = ctx.permissions[resource] ?? []
    if (!allowed.includes(action) && !allowed.includes('*')) {
      return res.status(403).json(problemDetail(403, `API key lacks permission: ${scope}`, req.correlationId))
    }
